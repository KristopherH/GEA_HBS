...
namespace Phyre
{
	namespace PSamplesCommon
	{
		...
		// Description:
		// Represents the base class for dialogs on PlayStation(R)4.
		class PDialogOrbis : public PDialogBase
		{
		protected:
			PResult initialize(const PChar *message, PDialogType type);
			PResult terminate(bool waitForExit);
		...
		// Description:
		// Represents the base class for progress bar dialogs on PlayStation(R)4.
		class PProgressBarDialogOrbis : public PDialogOrbis
		{
		protected:
			PResult initialize(const PChar *message);
			void update(PUInt32 percent);
