...
#include "PhyreSamplesCommonDialogs.h"
...
PHYRE_PRAGMA_COMMENT_LIB(SceCommonDialog_stub_weak)
PHYRE_PRAGMA_COMMENT_LIB(SceMsgDialog_stub_weak)
...
namespace Phyre
{
	namespace PSamplesCommon
	{
		...
		{
			// Description:
			// A helper class to simplify initializing the dialog system and handling unloading on shutdown.
			class PDialogOrbisInitializer
			{
			protected:
				bool m_initialized;	// A flag that indicates whether the dialog system has been initialized.

			public:

				// Description:
				// The constructor for the PDialogOrbisInitializer class.
				PDialogOrbisInitializer()
					: m_initialized(false)
				{
				}

				// Description:
				// The destructor for the PDialogOrbisInitializer class. This function unloads the message dialog module if it has been loaded.
				~PDialogOrbisInitializer()
				{
					if(m_initialized)
					{
				...
				// When this function is first called, it loads the required sysmodule and initializes the common dialog system.
				// The current message dialog is also initialized too.
				// Return Value List:
				// PE_RESULT_UNKNOWN_ERROR : A platform specific error occurred.
				// PE_RESULT_NO_ERROR: This dialog was initialized successfully.
				PResult initialize()
				{
					...
					if(!m_initialized)
					{
						result = sceSysmoduleLoadModule(SCE_SYSMODULE_MESSAGE_DIALOG);
						if(result != SCE_OK)
							return PHYRE_SET_LAST_ERROR(PE_RESULT_UNKNOWN_ERROR, "Error from sceSysmoduleLoadModule(SCE_SYSMODULE_MESSAGE_DIALOG) - 0x%08x", result);
						result = sceCommonDialogInitialize();
						if(result != SCE_OK)
							return PHYRE_SET_LAST_ERROR(PE_RESULT_UNKNOWN_ERROR, "Error from sceCommonDialogInitialize() - 0x%08x", result);

						m_initialized = true;
					}
					result = sceMsgDialogInitialize();
					if(result != SCE_OK)
						return PHYRE_SET_LAST_ERROR(PE_RESULT_UNKNOWN_ERROR, "Error from sceMsgDialogInitialize() - 0x%08x", result);
					return PE_RESULT_NO_ERROR;
				}
			};

			static PDialogOrbisInitializer s_dialogOrbisInitializer;	// Static instance to be used for initialization and termination via destruction.
		};

		//
		// PDialogOrbis implementation
		//
		...
		// PE_RESULT_UNKNOWN_ERROR : A platform specific error occurred.
		// PE_RESULT_NO_ERROR: This dialog was initialized successfully.
		PResult PDialogOrbis::initialize(const PChar *message, PDialogType type)
		{
			PHYRE_TRY(PInternal::s_dialogOrbisInitializer.initialize());
			...
			PInt32 res = sceMsgDialogOpen(&msgParam);
			if (res != 0)
				return PHYRE_SET_LAST_ERROR(PE_RESULT_UNKNOWN_ERROR, "Error from sceMsgDialogOpen = 0x%08x", res);
			return PE_RESULT_NO_ERROR;
		}
		...
		// Terminates this dialog.
		// Arguments:
		// waitForExit : A flag that indicates whether this method should wait for the dialog to close.
		// Return Value List:
		// PE_RESULT_UNKNOWN_ERROR : A platform specific error occurred.
		// PE_RESULT_NO_ERROR: This dialog was terminated successfully.
		PResult PDialogOrbis::terminate(bool waitForExit)
		{
			SceCommonDialogStatus dialogStatus = sceMsgDialogUpdateStatus();
			if(dialogStatus == SCE_COMMON_DIALOG_STATUS_RUNNING)
			{
				SceInt32 ret = sceMsgDialogClose();
				if(ret != 0)
			...
			if(waitForExit && dialogStatus == SCE_COMMON_DIALOG_STATUS_RUNNING)
			{
				while(sceMsgDialogUpdateStatus() != SCE_COMMON_DIALOG_STATUS_FINISHED)
					render();
				SceInt32 ret = sceMsgDialogTerminate();
				if(ret != 0)
					return PHYRE_SET_LAST_ERROR(PE_RESULT_UNKNOWN_ERROR, "Error from sceMsgDialogTerminate = 0x%08x", ret);
				render(); // Clear the screen
			}
			return PE_RESULT_NO_ERROR;
		}
		...
		// Description:
		// The PlayStation(R)4 render implementation for the dialog.
		// This function calls sceMsgDialogGetResult() to check the status of the dialog.
		void PDialogOrbis::render()
		{
			if(sceMsgDialogUpdateStatus() == SCE_COMMON_DIALOG_STATUS_FINISHED)
			{
				// Get message dialog result
				SceMsgDialogResult   s_commonDialogResult;
				memset(&s_commonDialogResult, 0, sizeof(SceMsgDialogResult));
		...
		//
		// PProgressBarDialogOrbis implementation
		//

		// Description:
		// Initializes the progress bar dialog with a specific message.
		// Arguments:
		// message : The message to display on the progress bar dialog.
		// Return Value List:
		// PE_RESULT_UNKNOWN_ERROR : A platform specific error occurred.
		// PE_RESULT_NO_ERROR : The progress bar dialog was initialized successfully.
		PResult PProgressBarDialogOrbis::initialize(const PChar *message)
		{
			PHYRE_TRY(PInternal::s_dialogOrbisInitializer.initialize());
			...
			SceInt32 res = sceMsgDialogOpen(&msgParam);
			if (res != SCE_OK)
				return PHYRE_SET_LAST_ERROR(PE_RESULT_UNKNOWN_ERROR, "Error from sceMsgDialogOpen = 0x%08x", res);
			return PE_RESULT_NO_ERROR;
		}
		...
		// Description:
		// Updates the percentage complete value shown by the progress bar dialog.
		// Arguments:
		// percent : The new percentage complete value to display.
		void PProgressBarDialogOrbis::update(PUInt32 percent)
		{
			sceMsgDialogProgressBarSetValue(SCE_MSG_DIALOG_PROGRESSBAR_TARGET_BAR_DEFAULT, percent);
		}
