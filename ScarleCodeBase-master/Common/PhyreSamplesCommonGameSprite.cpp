...
#include "PhyreSamplesCommonGameSprite.h"
	...
	namespace PSamplesCommon
	{
		...
		PResult PGameSprite::animate(float elapsedTime) 
		{
			...
			if (m_animInfo && m_active && m_time >= m_timeInterval)
			{
				...
				PSprite::PTextureAtlasInfo *atlas=m_animInfo->m_textureAtlasInfo;
				if (!atlas)
		...
		PResult PGameSprite::addToCollection(Phyre::PSprite::PSpriteCollection *collection, Phyre::PSprite::PSpriteAnimationInfo &animInfo, PUInt32 index, float x, float y, float sizeX, float sizeY)
		{
			...
			PSprite::PTextureAtlasInfo *atlas=animInfo.m_textureAtlasInfo;
			if (!atlas)
				return PE_RESULT_UNKNOWN_ERROR;
			PSprite::PSubTextureInfo &spriteInfo = atlas->m_subTextures[spriteIndex];
		...
		// Description:
		// The constructor for the PGameSprite class.
