...
namespace Phyre
{
	namespace PSamplesCommon
	{
		...
		PResult PFIOSInterfacePSP2::MountArchive(const PChar *name, PFIOSArchive &archive)
		{
			...
			// Mount the archive.
			int err = sceFiosArchiveMountSync(NULL, &archive.m_fileHandle, filenameUsed, PSerialization::PStreamFilePSP2::GetFIOS2ArchiveMountPoint(), PInternal::s_mountBuffer, NULL);
			if(err != SCE_FIOS_OK)
		...
		PResult PFIOSInterfacePSP2::Terminate()
		{
